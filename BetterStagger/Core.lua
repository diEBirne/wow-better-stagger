...
function addon:UpdateSound(staggerPercent)
    ...
    if soundConfig.soundFile then
        PlaySound(soundConfig.soundFile)
    else
...
function addon:Update(force)
    ...
    self:UpdateText(staggerPercent, scaleMaximum)
...
function addon:ToggleTestMode()
    ...
    if self.testMode then
        print("|cff00ff00Better Stagger:|r Test mode enabled (" .. self.testStaggerPercent .. "%).")
    else
